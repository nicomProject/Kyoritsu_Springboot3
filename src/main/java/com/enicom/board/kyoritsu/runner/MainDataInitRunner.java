...
public class MainDataInitRunner implements ApplicationRunner {
    ...
    private void configureMainMenu() {
        ...
        MainMenu recruit = MainMenu.builder().order(4).name("채용정보").nameEnglish("Recruitment").nameJapanese("採用情報").type(MainMenuType.GROUP).build();
...
//        if(!mainMenuStoredList.containsKey("채용지원")) mainMenuList.add(MainMenu.builder().order(4).menu(recruit).url("/recruit/apply").name("채용지원").nameEnglish("Recruitment<br>support").nameJapanese("採用支援").type(MainMenuType.GENERAL).build());
        if(!mainMenuStoredList.containsKey("채용문의")) mainMenuList.add(MainMenu.builder().order(4).menu(recruit).url("/recruit/inquire").name("채용문의").nameEnglish("Recruitment<br>Inquiry").nameJapanese("採用お問い合わせ").type(MainMenuType.GENERAL).build());
